...
export async function isCommandAvailable(nodevisor: Nodevisor, command: string): Promise<boolean> {
  switch (await nodevisor.platform()) {
    case Platform.WINDOWS:
      return await nodevisor.$`powershell -command "Get-Command ${command} -ErrorAction SilentlyContinue"`.toBoolean(
        true,
      );
    default:
      return await nodevisor.$`command -v ${command}`.toBoolean(true);
  }
}
