...
export default class CommandBuilder implements PromiseLike<CommandOutput> {
  ...
  constructor(connection: Connection, options: CommandBuilderOptions = {}) {
    const { command = [], quote, noThrow = false, as, env, stdin, signal, timeout } = options;
    ...
    if (this.quote) {
      ...
      if (signal) {
        signal.addEventListener('abort', () => {
          handleAbort('Command aborted');
        });
      }

      const command = this.toString();
      try {
        return await this.connection.exec(command, { stdin, signal: abortController.signal });
      } catch (error) {
        if (this.#noThrow) {
