export default class DockerfileStage {
  ...
  constructor(image: string, name: string) {
    ...
    this.from(image, name);
  }

  private from(image: string, as?: string) {
    this.lines.push(`FROM ${image} as ${as}`);
    return this;
  }
  ...
  copy(
    ...
    if (options?.from) {
      const from = options.from instanceof DockerfileStage ? options.from.name : options.from;
      if (!from) {
  ...
  if(condition: boolean, cb: (stage: DockerfileStage) => void) {
    if (condition) {
      cb(this);
    }
